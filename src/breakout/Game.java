...
public class Game extends JPanel {
	...
	private void init() {
		...
		for (int rows = 0; rows < 8; rows++) {
			if (rows == 0) currColor = Color.red;
			if (rows == 2) currColor = Color.orange;
			if (rows == 4) currColor = Color.yellow;
			if (rows == 6) currColor = Color.blue;
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < blocks.size(); i++) {
			if (blocks.get(i).alive()) blocks.get(i).paint(g2d);
		}
	}
